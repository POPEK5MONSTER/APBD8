...
namespace Exercise6
{
    public static class LinqTasks
    {
        ...
        {
            IEnumerable<Emp> result = Emps.Where(emp1 => emp1.Salary == Emps.Max(emp2 => emp2.Salary));
            return result;
        }
        ...
        public static Emp Task9()
        {
            Emp result = Emps.Where(emp => emp.Job == "Frontend programmer")
                             .OrderByDescending(emp => emp.HireDate).Take(1).First();
            return result;
        }
        ...
        {
            IEnumerable<object> result = Depts.GroupJoin(Emps, dept => dept.Deptno, emp => emp.Deptno, (dept, emp) => new
            {
                name = dept.Dname,
        ...
        {
            IEnumerable<Emp> result = CustomExtensionMethods.getBoss();
            return result;
        }
        ...
        public static IEnumerable<Dept> Task14()
        {
            IEnumerable<Dept> result = Depts
                .GroupJoin(Emps, dept => dept.Deptno, emp => emp.Deptno
                    , (dept, emps) => new
                    {
                        dept,
                        Employees = emps.Count()
                    })
                .Where((dept) => dept.Employees == 5 || dept.Employees==0)
    ...
    public static class CustomExtensionMethods
    {
        ...
        public static IEnumerable<Emp> getBoss()
        {
            var result = LinqTasks.Emps.GroupJoin(LinqTasks.Emps, emp1 => emp1.Empno,
                    emp2 => emp2.Mgr == null ? -1 : emp2.Mgr.Empno,
                    (emp1, emp2) => new
                    {
                        Manager = emp1,
                        Empleeyes = emp2.Count()
                    })
                .Where(x => x.Empleeyes >= 1)
                .Select(y => y.Manager)
